<script lang="ts">
    ...
    const dispatch = createEventDispatcher<{ deleteNote: string }>(); 
...
<div class="bg-white rounded-lg shadow-md p-6 flex flex-col justify-between h-fit max-w-full overflow-x-auto">
    {#if isEditing}
        <NoteForm note={note} isEditing={true} on:noteUpdated={handleNoteUpdated} on:cancelEdit={handleCancelEdit} />
    {:else}
        <div>
